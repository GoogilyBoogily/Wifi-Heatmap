...
public class Gradient {
	...
	private HashMap<Integer, ColorInterval> generateColorIntervals() {
		...
		for (int i = 1; i < mColors.length; i++) {
			colorIntervals.put(((int) (mColorMapSize * mStartPoints[i - 1])), new ColorInterval(mColors[i - 1], mColors[i], (mColorMapSize * (mStartPoints[i] - mStartPoints[i - 1]))));
		}
		...
		if (mStartPoints[mStartPoints.length - 1] != 1) {
			...
			colorIntervals.put(((int) (mColorMapSize * mStartPoints[i])), new ColorInterval(mColors[i], mColors[i], mColorMapSize * (1 - mStartPoints[i])));
		}
