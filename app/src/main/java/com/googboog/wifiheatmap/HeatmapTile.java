...
public class HeatmapTile implements TileProvider {
	/**
	...
	public static class Builder {
		...
		public HeatmapTile build() {
			...
			return new HeatmapTile(this);
		}
	}

	private HeatmapTile(Builder builder) {
	...
	public Tile getTile(int x, int y, int zoom) {
		...
		for (WeightedLatLng w : points) {
			...
			Log.d("Point", "(" + bucketX + ", " + bucketY + ")" + ", " + w.getIntensity());

			//RadialGradient g = new RadialGradient(bucketX, bucketY, mRadius, generateColorIntensity((float)w.getIntensity()), Color.TRANSPARENT, Shader.TileMode.CLAMP);
			Paint gp = new Paint();
			gp.setColor(generateColorIntensity((float) w.getIntensity()));
			//gp.setShader(g);
			gp.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC));
			myCanvas.drawCircle(bucketX, bucketY, mRadius, gp);
		}
		...
		for (WeightedLatLng w : wrappedPoints) {
			...
			Log.d("Wrapped Point", "(" + bucketX + ", " + bucketY + ")" + ", " + w.getIntensity());
		...
		//colorize();
		//myCanvas.drawBitmap(backbuffer, 0, 0, new Paint(Paint.ANTI_ALIAS_FLAG));
		//return convertBitmap(backbuffer);
	}
	...
	public void initBit() {
		backbuffer = Bitmap.createBitmap(TILE_DIM, TILE_DIM, Bitmap.Config.ARGB_8888);
		myCanvas = new Canvas(backbuffer);
		myCanvas.drawColor(Color.TRANSPARENT);
		Paint p = new Paint();
		p.setStyle(Paint.Style.FILL);
		p.setColor(Color.TRANSPARENT);
		myCanvas.drawRect(0, 0, TILE_DIM, TILE_DIM, p);
		myCanvas.drawBitmap(backbuffer, 0, 0, new Paint(Paint.ANTI_ALIAS_FLAG));
	}

	public int generateColorIntensity(float intensity) {
		int color1 = Color.argb(100, 0, 0, 0); //Color.rgb(102, 225, 0)
		int color2 = Color.argb(255, 0, 0, 0); //Color.rgb(255, 0, 0)
	...
	private void colorize() {
		int[] pixels = new int[backbuffer.getHeight() * backbuffer.getWidth()];
		backbuffer.getPixels(pixels, 0, backbuffer.getWidth(), 0, 0, backbuffer.getWidth(), backbuffer.getHeight());
		...
		backbuffer.setPixels(pixels, 0, backbuffer.getWidth(), 0, 0, backbuffer.getWidth(), backbuffer.getHeight());
	} // end colorize()
	...
	public double[][] generateHeatmapData(double[][] grid) {
				...
				if (val != 0) {
					...
					for (x2 = initial; x2 < xUpperLimit; x2++) {
						...
						if(intermediate[x2][y] != 0) {
							//intermediate[x2][y] = (intermediate[x2][y] + (val * kernel[x2 - (x - radius)])) / 2;
							intermediate[x2][y] = Math.max(intermediate[x2][y], val * kernel[x2 - (x - radius)]);
						} else {
				...
				if (val != 0) {
					...
					for (y2 = initial; y2 < yUpperLimit; y2++) {
						...
						if (outputGrid[x - radius][y2 - radius] != 0) {
							//outputGrid[x - radius][y2 - radius] = (outputGrid[x - radius][y2 - radius] + (val * kernel[y2 - (y - radius)])) / 2;
							outputGrid[x - radius][y2 - radius] = Math.max(outputGrid[x - radius][y2 - radius], val * kernel[y2 - (y - radius)]);
						} else {
